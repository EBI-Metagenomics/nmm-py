...
class CCodonTable:
    ...
    def __init__(
        ...
        if imm_codont is None:
            ...
            self.__imm_codont = lib.nmm_codont_create(alphabet.imm_abc)
        else:
            if alphabet is not None:
                raise ValueError("`alphabet` is not `None`")

            self.__imm_codont = imm_codont

        if self.__imm_codont == ffi.NULL:
            raise RuntimeError("`cdata` is NULL.")

    @property
    def nmm_codont(self) -> ffi.CData:
        return self.__imm_codont

    @property
    def imm_abc(self) -> ffi.CData:
        return lib.nmm_codont_get_abc(self.__imm_codont)

    def set_lprob(self, seq: bytes, lprob: float) -> None:
        if len(seq) != 3:
            raise ValueError("Codon must have three letters.")

        err: int = lib.nmm_codont_set_lprob(self.__imm_codont, create_codon(seq), lprob)
        if err != 0:
            s = seq.decode()
    ...
    def get_lprob(self, seq: bytes) -> float:
        ...
        return lib.nmm_codont_get_lprob(self.__imm_codont, create_codon(seq))

    def normalize(self) -> None:
        err: int = lib.nmm_codont_normalize(self.__imm_codont)
        if err != 0:
            raise RuntimeError("Normalization error.")

    def __del__(self):
        if self.__imm_codont != ffi.NULL:
            lib.nmm_codont_destroy(self.__imm_codont)
